...
## function is executed again to generated a new Model Fit    
    ...
    if (input$trainMethod==1) {
      modFit<-train(Species~., data=train_ds(), method=input$trainAlgorithm, verbose=FALSE)
    } else 
      if (input$trainMethod ==2) {
        modFit<-train(Species~., data=train_ds(), method=input$trainAlgorithm, trControl=trainControl(method="cv",5), verbose=FALSE)
      } else {
        modFit<-train(Species~., data=train_ds(), method=input$trainAlgorithm, trControl=trainControl(method="cv",10), verbose=FALSE)
      }
  })
